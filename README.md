...
> `/remake start (初始颜值值) (初始体质值) (初始智力值) (初始家境值) (初始快乐值)`
> 
> 默认下, 会随机分配
...
![talents](docs_img/talents-example.png)
...
![events](docs_img/life-example.png)
...
- 判断的成员属性，包括以下值
  ...
  - `RDM`  随机属性(从颜值,智力,体质,家境,快乐)中随机抽取
    ...
    >
    > 这些转化是**运行时转化的**，并不会在内存中存储这些临时事件，当然你也可以在配置 _`game.enableCacheTemporaryEvent`_ 中设定
- 判断的类型
  - 大于: `>`
  - 小于: `<`
  - 包含: `?[...]`
